...
async function completeImageTask(
  ...
  const defaultCompletionMessage = failedCount > 0
    ? `生成完成：成功 ${outputIds.length} 张，失败 ${failedCount} 张`
    : `生成完成，共 ${outputIds.length} 张图片`
  const completionMessage = options.successMessage ?? defaultCompletionMessage
  useStore.getState().showToast(completionMessage, failedCount > 0 && !options.successMessage ? 'error' : 'success')
  if (!isAgentTask(task)) {
    showTaskCompletionNotification('图像生成完成', options.notificationMessage ?? `${completionMessage}。`)
  }
}
