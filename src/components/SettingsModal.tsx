...
import { isApiProxyAvailable, isApiProxyLocked, isBackendTasksEnabled, readClientDevProxyConfig } from '../lib/devProxy'
...
import { ChevronDownIcon, CloseIcon, CopyIcon, PlusIcon, TrashIcon, CodeIcon, ExportIcon, ImportIcon, DragHandleIcon, LinkIcon } from './icons'
...
export default function SettingsModal() {
  ...
  const backendTasksEnabled = isBackendTasksEnabled()
  ...
  const apiUrlLocked = activeProviderIsOpenAICompatible && (backendTasksEnabled || apiProxyEnabled)
  const apiProxyControlLocked = activeProviderIsOpenAICompatible && (backendTasksEnabled || apiProxyLocked)
  ...
  const commitSettings = (nextDraft: AppSettings) => {
    const normalizedProfiles = nextDraft.profiles.map((profile) => {
      ...
      const shouldKeepEmptyBaseUrl = profile.provider !== 'fal' && (backendTasksEnabled || nextApiProxy) && !profile.baseUrl.trim()
              ...
              {activeProviderUsesApiUrl && (
                <label className="block">
                  ...
                  <input
                    ...
                    type="text"
                    disabled={apiUrlLocked}
                    placeholder={activeProfile.provider === 'fal' ? DEFAULT_FAL_BASE_URL : DEFAULT_SETTINGS.baseUrl}
                    className={`w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2.5 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50 ${apiUrlLocked ? 'opacity-50 cursor-not-allowed' : ''}`}
                  />
                  <div data-selectable-text className="mt-1.5 min-h-[22px] flex items-center text-xs text-gray-500 dark:text-gray-500">
                    {backendTasksEnabled ? (
                      <span className="text-yellow-600 dark:text-yellow-500">后端保持连接已启用，实际请求目标由后端环境变量决定，此处设置被忽略。</span>
                    ) : apiProxyEnabled ? (
                ...
                <div className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        if (!apiProxyControlLocked) updateActiveProfile({ apiProxy: !activeProfile.apiProxy }, true)
                      }}
                      disabled={apiProxyControlLocked}
                      className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${apiProxyChecked ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'} ${apiProxyControlLocked ? 'cursor-not-allowed opacity-70' : ''}`}
                      role="switch"
                  ...
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    {backendTasksEnabled ? '后端保持连接已启用，请求由后端转发到上游 API；此开关不会影响实际请求。' : apiProxyLocked ? '部署端已锁定代理开启，请求经服务器转发到上游 API，上方 URL 设置将失效。' : '开启后请求经服务器转发到上游 API，可绕过浏览器跨域限制，上方 URL 设置将失效。'}
                  </div>
                </div>
              )}
